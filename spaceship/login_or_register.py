...
class LoginOrRegister:
  ...
  @classmethod
  def login(
      cls, email: Optional[str] = None, password: Optional[str] = None, oauth_user_info: Optional[Dict[str, Any]] = {}) -> User:
    """Logs a user in"""
  ...
  @classmethod
  def perform(cls, email=None, name=None, password=None, oauth_user_info={}, as_captain=False):
    """registers or logs the user in"""
    # no matter what, we require an email address; default to the oauth one if available
    email = oauth_user_info.get('email', email)
    if not email:
      raise cls.Error('email is required')

    # if the user already exists, log them in
    u = User.query.filter(User.email == email).first()
    if u:
      return cls.login(email, password, oauth_user_info)
